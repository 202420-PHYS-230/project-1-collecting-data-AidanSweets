...
We measured the voltage at two points in a resistor network to validate the behavior of the circuit. We found the ratio of voltages was: $0.665\pm 0.001$ close to the expected value of $0.667$. We also measured the decay over time of the voltage on a capacitor. We used that to estimate the decay time constant and found that to be $\tau = .06824 \pm -.0003620\mathrm{~ms}$, very close to the expected value of $\tau =.06667\mathrm{~ms}$.
...
The goal of this project was to become famiar with all the tools we'll be using this semester in Laboratory Instrumentation and to make some real measurements, and analyze some real data. The project taught us how to use thonny, A microcontroller aware editor, to measure the voltages within the microcontroller. It also taught us what libraries we want to import into our projects, how to extract and insert data into the code, and How to use the curve_fit operator. So I would not change anything about this project because the code did everything it was written to do and teach.
